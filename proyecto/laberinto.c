...
para convertir la entrada del usuario a minúscula usando 'tolower()'*/
#include <ctype.h>
...
/*Ejecuta el comando del sistema 'stty sane' para resetear la configuración
de la terminal después de que el programa termina*/
#include <stdlib.h>
...
/*Declara dos variables de tipo struct termios, una estructura definida en <termios.h>
que contiene todos los parámetros de configuración del terminal. Se usan para guardar y 
modificar la configuración del terminal en sistemas Unix/Linux/macOS*/
struct termios oldt; // Guarda la configuración original del terminal antes de modificarla.
struct termios newt; // Guarda la nueva configuración a aplicar (para capturar teclas sin ENTER y sin eco).
...
void restaurarTerminal(struct termios *oldt); // No devuelve nada, toma un puntero a la configuración original de terminal.
void configurarTerminal(struct termios *oldt, struct termios *newt); // No devuelve nada, toma un puntero a la configuración original de terminal y otro a la configuración nueva (modificada).
char obtenerTecla(struct termios *oldt, struct termios *newt); // Devuelve un 'char', toma un puntero a la configuración original de terminal y otro a la configuración nueva (modificada).
void mostrarMenuInicio(); // No devuelve nada, no toma parámetros. 
int calcularMinimosMovimientos(char laberinto[FILAS][COLUMNAS], int inicioX, int inicioY, int *salidaX, int *salidaY); // Devuelve un 'int', toma una array 'laberinto', coordenadas de inicio, y punteros a coordenadas de salida.
int cargarLaberinto(char laberinto[FILAS][COLUMNAS], const char *nombreArchivo); // Devuelve un 'int', toma un array 'laberinto' y el nombre del archivo.
void mostrarLaberinto(char laberinto[FILAS][COLUMNAS], int x, int y); // No devuelve nada, toma un array 'laberinto'' y las coordenadas del jugador.
void moverJugador(char laberinto[FILAS][COLUMNAS], int *x, int *y, char input); // No devuelve nada, toma un array 'laberinto', coordenadas del jugador, y un caractér como entrada.

// Definición de funciónes:

// Función para restaurar la configuración original del terminal.
// Devuelve la terminal a su configuración original tras terminar el juego,
// para que el usuario pueda continuar usándola después de jugar.
// Se llama desde main() antes de cada salida del programa para que la terminal vuelva a funcionar normalmente.
void restaurarTerminal(struct termios *oldt) {
    if (tcgetattr(STDIN_FILENO, oldt) == 0) { // Verifica que el terminal es accesible.
        tcsetattr(STDIN_FILENO, TCSANOW, oldt); // Aplica la configuración original.
    }
}
...
void configurarTerminal(struct termios *oldt, struct termios *newt) {
    tcgetattr(STDIN_FILENO, oldt); // Se obtiene la configuración actual de la entrada estándar y se la guarda en 'oldt'.
    *newt = *oldt; // Se copia la configuración actual ('oldt') a 'newt' para poder modificar solo los parámetros necesarios.
    newt->c_lflag &= ~(ICANON | ECHO); // Desactiva el modo canónico y eco.
    ...
    tcsetattr(STDIN_FILENO, TCSANOW, newt); // Aplica la nueva configuración a la terminal.
}
...
char obtenerTecla(struct termios *oldt, struct termios *newt) {
...
/* Imprime el menú principal del juego con las opciones "Jugar" y "Salir".
    ...
    while (frente < cola_tamano) { // El bucle continúa mientras haya nodos en la cola por procesar.
        ...
        for (int i = 0; i < 4; i++) { // Para cada una de las cuatro direcciones posibles:
            ...
            if (nx >= 0 && nx < FILAS && ny >= 0 && ny < COLUMNAS && !visitado[nx][ny] && laberinto[nx][ny] != '#') {
                ...
                cola[cola_tamano++] = (Nodo){nx, ny, dist + 1}; // - Se agrega a la cola con una distancia incrementada en 1.
            }
        }
    }
...
void moverJugador(char laberinto[FILAS][COLUMNAS], int *x, int *y, char input) {
    int movido = 0; // Se crea una variable para marcar si el jugador se movió exitosamente y se inicializa en 0.
    switch (input) { // Utiliza un 'switch' para evaluar el valor de 'input' y ejecutar el movimiento correspondiente según la tecla presionada.
        ...
        -Se decrementa *x (movimiento hacia arriba).
        -Se cambia la variable 'movido' a 1. */
        case 'w': if (*x > 0 && laberinto[*x - 1][*y] != '#') { (*x)--; movido = 1; } break;
        /* Mover hacia  abajo :
        -Verifica que el jugador no esté en la última fila.
        -Verifica que la celda de abajo no sea un muro.
        Si ambas condiciones se cumplen:
        -Se incrementa *x (movimiento hacia abajo).
        -Se cambia la variable 'movido' a 1. */
        case 's': if (*x < FILAS - 1 && laberinto[*x + 1][*y] != '#') { (*x)++; movido = 1; } break;
        /* Mover hacia la izquierda : 
        -Verifica que el jugador no esté en la primera columna.
        -Verifica que la celda de la izquierda no sea un muro.
        Si ambas condiciones se cumplen:
        -Se decrementa *y (movimiento hacia la izquierda).
        -Se cambia la variable 'movido' a 1. */
        case 'a': if (*y > 0 && laberinto[*x][*y - 1] != '#') { (*y)--; movido = 1; } break;
        /* Mover hacia la derecha :
        -Verifica que el jugador no esté en la última columna.
        -Verifica que la celda de la derecha no sea un muro.
        Si ambas condiciones se cumplen:
        -Se incrementa *y (movimiento hacia la derecha).
        -Se cambia la variable 'movido' a 1. */
        case 'd': if (*y < COLUMNAS - 1 && laberinto[*x][*y + 1] != '#') { (*y)++; movido = 1; } break;
        // Regresar al menú :
    ...
    if (movido) {  // Si movido se cambió a 1 (se movió exitosamente).
        movimientos++; // incrementa el contador global de movimientos.
    }
}
...
int main() {
    ...
    while (1) { 
        ...
        if (opcion != '1' && opcion != '2') { // Si el usuario ingresa algo distinto de '1' o '2'...
            ...
            // Limpia el resto del buffer de entrada.
        ...
        if (opcion == '2') { // Si el usuario selecciona '2'...
            ...
            Esto da tiempo al usuario para leer el mensaje antes de cerrar. */
            getchar();

            restaurarTerminal(&oldt); // Llamada para restaurar el terminal a su estado original.
            // Doble seguridad: para asegurar que la terminal funcione correctamente.
            system("stty sane"); // Ejecuta un comando del sistema operativo que restaura la terminal a valores por defecto.
            return 0; // Sale de la función main() y termina el programa.
        }
        ...
        if (!cargarLaberinto(laberinto, "laberinto.txt")) {
            printf("Verifique el archivo antes de continuar.\n");
            printf("Presione cualquier tecla para reintentar.\n");
            getchar(); // Da tiempo al usuario para leer el mensaje antes de cerrar.
            continue;
        }
        ...
        while (1) {
            ...
            char input = obtenerTecla(&oldt, &newt); // Llamada para obtener el input del usuario.
            ...
            if (input == 'q') { // Verificar si el jugador presionó 'Q' para regresar al menú.
